...
public class DominantFrequencyCounter_CARLOS_v2 : MonoBehaviour
{
    //Datos reales
    ...
    float frequency;
    float totalCadence;
    List<float> lastFourFrequencies = new List<float>(); // Lista para almacenar los últimos cuatro resultados
    ...
    void Start()
    {
        ...
        using (StreamWriter writer = new StreamWriter(filePath))
        {
            ...
            for (int i = 0; i < data.Count - 128; i += 32)
            {
                ...
                // Añadir el resultado a la lista y mantener sólo los últimos 4 resultados
                lastFourFrequencies.Add(frequency);
                if (lastFourFrequencies.Count > 4)
                {
                    lastFourFrequencies.RemoveAt(0);
                }

                // Calcular la media de las frecuencias almacenadas
                float averageFrequency = 0;
                foreach (float freq in lastFourFrequencies)
                {
                    averageFrequency += freq;
                }
                averageFrequency /= lastFourFrequencies.Count;
                ...
                Debug.Log("[FFT] Frecuencia media de los últimos 4 intervalos: " + averageFrequency);
                Debug.Log("[FFT] Cadencia media de los últimos 4 intervalos: " + averageFrequency * 60f);
                writer.WriteLine($"{fileName};{startInterval} - {endInterval};{averageFrequency};{averageFrequency * 60f}");

                // Si se alcanza el intervalo final
                if (endInterval == 1580)
                {
                    Debug.Log("FIN");
                    ...
                    writer.WriteLine($"Cadencia total;128(300-1428);{frequency};{totalCadence}");
                    break;
                }
            }
        }
