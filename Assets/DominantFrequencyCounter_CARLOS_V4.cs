...
public class DominantFrequencyCounter_CARLOS_V4 : MonoBehaviour
{
    //Datos reales
    ...
    void Start()
    {
        ...
        {
            writer.WriteLine("Nombre;Intervalo;FFT;Cadencia");

            // Evaluar manualmente los primeros 128, 256 y 384 datos
            EvaluateInitialIntervals(writer, 128);
            EvaluateInitialIntervals(writer, 256);

            // Procesar los datos en intervalos dinámicos de 512 frames a partir de 512
    ...
    void EvaluateInitialIntervals(StreamWriter writer, int intervalLength)
    {
        if (data.Count >= intervalLength)
        {
            List<float> block = data.GetRange(0, intervalLength);
            inputData = block.ToArray();
            DoFFT(inputData);
            Debug.Log($"----FRECUENCIA MEDIA DATOS PRUEBA: {intervalLength} FRAMES-----");
            Debug.Log("[FFT] Frecuencia dominante: " + frequency);
            Debug.Log("[FFT] Cadencia: " + frequency * 60f);
            writer.WriteLine($"{fileName};300 - {300 + intervalLength};{frequency};{frequency * 60f}");
        }
    }
