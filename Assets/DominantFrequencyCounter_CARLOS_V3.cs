...
public class DominantFrequencyCounter_CARLOS_V3 : MonoBehaviour
{
    //Datos reales
    ...
    void Start()
    {
        ...
        using (StreamWriter writer = new StreamWriter(filePath))
        {
            ...
            // Procesar los datos en intervalos de 32 frames
            for (int i = 0; i < data.Count; i += 32)
            {
                int blockSize = Mathf.Min(data.Count - i, 512);

                // Solo procesar si el tamaño del bloque es un múltiplo de 128
                if (blockSize % 128 == 0)
                {
                    ...
                    if (blockSize == 512 && endInterval >= 1580)
                    {
                        Debug.Log("FIN");
